...
/**
 * Data Access Object cho User entity Model layer - DAO pattern
 */
...
//     */
    ...
    public static void main(String[] args) {
        try (Connection conn = DBConnection.getInstance().getConnection()) {
            ...
            ps.setString(1, "lan@owner.com");

            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                String password = rs.getString("password");
                System.out.println("Tìm thấy user. Password trong DB: " + password);
            } else {
                System.out.println("Không tìm thấy user với email này.");
            }

        } catch (Exception e) {
            System.out.println("Lỗi khi kết nối hoặc query!");
            e.printStackTrace();
        }
    }
}
